...
function HeroFrame({ frame, index, priority, register, onFocusChange }: FrameProps) {
  const asset = media[frame.media];

  const onFrame = useCallback(
    (element: HTMLDivElement | null) => register(element, index),
    [register, index],
  );
  ...
  return (
    <div
      ...
      className="shrink-0"
      style={{ marginLeft: index === 0 ? 0 : 'max(-2.4vw, -30px)' }}
    >
      <ShaderImage
        id={asset.id}
        asset={asset}
        ...
        className="w-[19vw] max-w-[228px] bg-obsidian/5 shadow-[0_18px_50px_-30px_rgba(21,21,21,0.6)] sm:w-[17vw]"
        style={{
...
export function Hero() {
  ...
  const onFocusChange = useCallback((index: number | null) => {
    ...
    hero.frames.forEach((frame, i) => {
      setSurfaceGray(media[frame.media].id, index === null || index === i ? 0 : 0.42);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const state = hero.frames.map(() => ({ x: 0, y: 0, r: 0, s: 1 }));
    let raf = 0;
    ...
    const tick = () => {
      ...
      hero.frames.forEach((frame, index) => {
        ...
        const targetX =
          relX * 34 * frame.depth +
          (focus !== null && !isFocused ? Math.sign(index - focus) * 16 : 0);
